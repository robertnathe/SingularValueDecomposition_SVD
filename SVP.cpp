...
int print_results (int nrows_A, int ncols_A, int nrows_B, int ncols_B, double array_U[], double D[], double array_V[], double X[])
{
  int i, j;
  printf ("******************** Solve Ax = B ********************\n\n");
  printf ("where U = \n");
...
int Parameters_Solve (double array_A[],  int nrows_A, int ncols_A, int ncols_B, double array_U[], double D[], double array_V[], double dummy_array[], double B[], double X[])
{
  int i, j;
  double A[nrows_A][ncols_A]  = {0};
  double U[nrows_A][ncols_A] = {0};
  double V[nrows_A][ncols_A]  = {0};
  B[ncols_B] = {0};
  X[ncols_B] = {0};
  fprintf (stdout, "\n");
  for (i = 0; i < nrows_A; i++)
...
// Destroys A and the Solution is returned in B.   //
  ...
  Parameters_Solve (&array_A[0], nrows_A, ncols_A, ncols_B, &array_U[0], &D[0], &array_V[0], &dummy_array[0], &B[0], &X[0]);
  print_results (nrows_A, ncols_A, nrows_B, ncols_B, &array_U[0], &D[0], &array_V[0], &X[0]);
  return;
}
int main ()
{
