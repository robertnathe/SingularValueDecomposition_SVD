...
static void print_vector (double *X, int ncols);
static void print_matrix (double *X, int nrows, int ncols);
int Parameters_Solve (double *A, int nrows_A, int ncols_A, int nrows_B, int ncols_B, double array_U[], double D[], double *V, double B[], double X[]);
...
static void print_vector (double *B, int ncolS)
{
  printf ("\n\n");
  int i, ncols;
  for (i = 0; i < ncols; i++)
                      ...
                      double* singular_values, double* V, double* dummy_array)
{
   ...
   if (Givens_Reduction_to_Diagonal_Form( nrows, ncols, U, V, singular_values, dummy_array ) < 0) return -1;
...
//                     double tolerance, int nrows, int ncols,                //
...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
void Singular_Value_Decomposition_Inverse(double* U, double* D, double* V, double tolerance, int nrows, int ncols, double *A_Pseudo_Inverse)
{
   int i,j,k;
   double *pu, *pv, *pa;
   double dum;
   dum = DBL_EPSILON * D[0] * (double) ncols;
   if (tolerance < dum) tolerance = dum;
   for ( i = 0, pv = V, pa = A_Pseudo_Inverse; i < ncols; i++, pv += ncols)
...
////////////////////////////////////////////////////////////////////////////////
void
Zero_Vector (double *X, int ncols)
{
  for (; ncols > 0; ncols--)
    *X++ = 0.0;
}
int print_results (double array_A[], int nrows_A, int ncols_A, int nrows_B, int ncols_B, double array_U[], double D[], double array_V[], double dummy_array[], double B[], double X[])
{
  int i, j;
  printf ("******************** Solve Ax = B ********************\n\n");
  printf ("where U = \n");
  for (i = 0; i < nrows_A; i++)
    {
      for (j = 0; j < ncols_A; j++)
	{
	  printf ("%6.4f   ", array_U[i * nrows_A + j]);
	}
      printf ("\n");
    }
  printf ("\n");
  ...
  for (i = 0; i < nrows_A; i++)
  {
      for (j = 0; j < ncols_A; j++)
	{
	  printf ("%6.4f   ", array_V[i* nrows_A + j]);
	}
      printf ("\n");
  }
  printf ("\n");
...
int Parameters_Solve (double array_A[],  int nrows_A, int ncols_A, int nrows_B, int ncols_B, double array_U[], double D[], double array_V[], double dummy_array[], double B[], double X[])
{
  int i, j;
  double A[nrows_A][ncols_A]  = {0}, U[nrows_A][ncols_A] = {0}, V[nrows_A][ncols_A]  = {0};
//  int pivot[ncols_B] = { 0 };
  B[ncols_B] = {0};
  X[ncols_B] = {0};
  fprintf (stdout, "\n");
  for (i = 0; i < nrows_A; i++)
  {
      for (j = 0; j < ncols_A; j++)
	{
	  A[i][j] = array_A[i * nrows_A + j];
//	  U[i][j] = array_A[i * nrows_A + j];
//	  V[i][j] = array_A[i * nrows_A + j];
	}
  }
  double tolerance = 0.000001;
  double A_Pseudo_Inverse[nrows_A][ncols_A];
  // Process data here.
  Singular_Value_Decomposition(&A[0][0], nrows_A, ncols_A, &U[0][0], &D[0], &V[0][0], &dummy_array[0]);
  Singular_Value_Decomposition_Solve(&U[0][0], &D[0], &V[0][0],  tolerance, nrows_A, ncols_A, &B[0], &X[0]);
  for (i = 0; i < nrows_A; i++)
  {
      for (j = 0; j < ncols_A; j++)
	{
	  array_A[i * nrows_A + j] = A[i][j];
	  array_U[i * nrows_A + j] = U[i][j];
	  array_V[i * nrows_A + j] = V[i][j];
	}
  }
  Singular_Value_Decomposition_Inverse(&A[0][0], &D[0], &V[0][0], tolerance, nrows_A, ncols_A, &A_Pseudo_Inverse[0][0]);
  printf ("The pseudo-inverse of A = UDV' is A_Pseudo_Inverse  = \n");
  print_matrix( (double*) A_Pseudo_Inverse, nrows_A, ncols_A);
  double array_A_Pseudo_Inverse[nrows_A*ncols_A];
  for (i = 0; i < nrows_A; i++)
  {
      for (j = 0; j < ncols_A; j++)
	{
	  array_A_Pseudo_Inverse[i * nrows_A + j] = A_Pseudo_Inverse[i][j];
	}
  }
  return 0;
}
...
Input_matrix_A (void)
{
  ...
  int dimArray[3];
  int nrows_A, ncols_A, number_of_entries_A;
  int nrows_B, ncols_B, number_of_entries_B;
  int i_index, j_index;
  double value;
  int i;
  if (myFile == NULL)
  ...
  Zero_Matrix (&A[0][0], nrows_A, ncols_A);
//    print_matrix( (double*) A, nrows_A, ncols_A);
  ...
  nrows_B = dim_B_Array[0];
  ncols_B = dim_B_Array[1];
  number_of_entries_B = dim_B_Array[2];
  double B_temporary[nrows_B][ncols_B];
  double B[ncols_B], X[ncols_B];
  Zero_Vector (&B[0], ncols_B);
  Zero_Matrix (&B_temporary[0][0], nrows_B, ncols_B);
//  print_matrix( (double*) A, nrows_A, ncols_A);
  ...
  fclose (myFile2);
  double U[nrows_A][ncols_A];
  Zero_Matrix (&U[0][0], nrows_A, ncols_A);
  int j, kk;
  kk = nrows_A * ncols_A;
  double array_A[kk];
  Zero_Vector (&array_A[0], kk);
for (i = 0; i < nrows_A; i++)
  {
      for (j = 0; j < ncols_A; j++)
	{
  ...
  printf ("Prog: SVD.cpp - The program solves the matrix AX = B using SVD. \n");
// Save matrices A and vector B, the routine Gaussian_Elimination() //
// Destroys A and the Solution is returned in B.   //
//  double B[ncols_B];
  double array_U[nrows_A*ncols_A], array_V[nrows_A*ncols_A];
  double D[ncols_A], dummy_array[ncols_A];
  double A_Pseudo_Inverse[nrows_A][ncols_A];
  for (i = 0; i < nrows_A; i++)
  {
      for (j = 0; j < ncols_A; j++)
	{
	  array_U[i * nrows_A + j] = array_A[i * nrows_A + j];
	  array_V[i * nrows_A + j] = array_A[i * nrows_A + j];
	}
  }
  Parameters_Solve (&array_A[0], nrows_A, ncols_A, nrows_B, ncols_B, &array_U[0], &D[0], &array_V[0], &dummy_array[0], &B[0], &X[0]);
  print_results (&array_A[0], nrows_A, ncols_A, nrows_B, ncols_B, &array_U[0], &D[0], &array_V[0], &dummy_array[0], &B[0], &X[0]);
  return;
}
int main ()
{
