...
static void print_matrix (double *X, int num_rows, int num_cols);
void Zero_Matrix(double* X, int num_rows, int num_cols);
void Zero_Vector(double* X, int num_cols);
int print_results (int num_rows, int num_cols, int num_rows_B, int num_cols_B, double array_U[], double D[], double array_V[], double X[]);
int Parameters_Solve (matrix<double> & AMatrix, int num_rows, int num_cols, double array_U[], double D[], double *V, double B[], double X[]);
...
void Singular_Value_Decomposition_Solve(double* U, double* D, double* V, double tolerance, int num_rows, int num_cols, double *B, double* x);
...
////////////////////////////////////////////////////////////////////////////////
//void
//Zero_Vector (double *X, int num_cols)
//{
...
int print_results (int num_rows, int num_cols, int num_rows_B, int num_cols_B, double array_U[], double D[], double array_V[], double X[])
{
  ...
  for (i = 0; i < num_rows_A; i++)
	...
	{
	  printf ("%6.4f   ", array_U[i * num_rows_A + j]);
	}
      printf ("\n");
    }
  printf ("\n");
  ...
  for (i = 0; i < num_rows_A; i++)
  {
      for (j = 0; j < num_cols_A; j++)
	{
	  printf ("%6.4f   ", array_V[i* num_rows_A + j]);
	}
      printf ("\n");
  }
  printf ("\n");
  ...
  // Need to verify that line 965 is correct. If it is not correct, then change num_rows_B and num_cols_B.
  fprintf (myFile3,"%d %d %d\n", 1, num_rows_B, num_cols_B);
  printf ("The solution is X = \n");
  //for (i = 0; i < num_cols_B; i++)
  // I don't know why I changed this, but it works fine now.
  for (i = 0; i < num_cols_A; i++)
  {
...
int Parameters_Solve (matrix<double> & AMatrix,  int num_rows, int num_cols, double array_U[], double D[], double array_V[], double dummy_array[], double B[], double X[])
{
  int i, j;
  int num_cols_B {0};
...
//  double A_Pseudo_Inverse[num_rows][num_cols];
  ...
  Singular_Value_Decomposition_Solve(&U[0][0], &D[0], &V[0][0],  tolerance, num_rows, num_cols, &B[0], &X[0]);
  for (int i = 0; i < num_rows; ++i) {
...
//  Singular_Value_Decomposition_Inverse(A, D, V, tolerance, num_rows, num_cols, &A_Pseudo_Inverse[0][0]);
  double A_Pseudo_Inverse[num_rows][num_cols];
  ...
  int Parameters_Solve(const matrix<double>& AMatrix, int num_rows, int num_cols,
                     double array_U[], double D[], double array_V[],
                     double dummy_array[], double B[], double X[]);
  return 0;
}
...
int main ()
{
  ...
  ReadMatrixMM(AMatrix, num_rows, num_cols);
  ...
  // Call Parameters_Solve function
  Parameters_Solve(AMatrix, num_rows, num_cols, array_U, &D[0], array_V, &dummy_array[0], &B[0], &X[0]);
  // Print results using the provided function
  print_results(num_rows, num_cols, num_rows, num_cols, array_U, &D[0], array_V, &X[0]);
