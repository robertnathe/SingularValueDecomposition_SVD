...
void writeMatrixMarketVector(const VectorX& vec, std::string_view filename) {
    std::ofstream file(filename.data());
    if (!file.is_open()) {
        throw std::runtime_error("Cannot write to file: " + std::string(filename));
    }
    file.exceptions(std::ofstream::badbit | std::ofstream::failbit);
    
    // Optimization: Buffer output in a stringstream to perform one large write operation.
    std::stringstream buffer;
    buffer << "%%MatrixMarket matrix array real general\n";
    buffer << vec.size() << " 1 "<< vec.size() << "\n";

    // Use std::to_chars for fast, non-locale-dependent floating point formatting.
    // DBL_DIG+2 precision ensures lossless round-trip for double.
    constexpr auto precision = std::numeric_limits<real_t>::max_digits10;
    std::array<char, 64> char_buf;

    for (Eigen::Index i = 0; i < vec.size(); ++i) {
        auto [ptr, ec] = std::to_chars(char_buf.data(), char_buf.data() + char_buf.size(), 
                                       vec(i), std::chars_format::scientific, precision);
        if (ec == std::errc()) {
            buffer.write(char_buf.data(), ptr - char_buf.data());
            buffer.put('\n');
        } else {
            // Fallback for safety, though should not be needed with a sufficient buffer size.
            buffer << std::scientific << std::setprecision(precision) << vec(i) << "\n";
        }
    }
    file << buffer.rdbuf();
}
...
int main() {
    ...
    try {
        ...
        MatrixMarketIO::writeMatrixMarketVector(x, "X_out.dat");
